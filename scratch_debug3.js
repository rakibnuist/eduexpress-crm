...
(async () => {
  ...
  function nextLeadId() {
    const row = db.prepare("SELECT lead_id FROM leads ORDER BY _rowid_ DESC LIMIT 1").get();
    if (!row) return 'LEAD-1001';
    const match = row.lead_id.match(/^LEAD-(\d+)$/);
  ...
  try {
    ...
    const destination = 'Bangladesh';
    const degree = null;

    let assigned_consultant = null;
    let assigned_employee_id = null;
    
    // Simulate finding channel consultant
    ...
    const params = leadParams({
      ...
      ad_name, channel_id, notes: 'Initial Inquiry'
    }, lead_id, 0);

    const info = db.prepare(LEAD_INSERT_SQL).run(params);
    const lead = db.prepare("SELECT * FROM leads WHERE id=?").get(info.lastInsertRowid);
    console.log("Success! Created Lead:", lead);
  } catch(e) {
