...
try {
    ...
    $res = $db->query("SELECT m.id, m.conversation_id, m.body, m.direction, m.created_at, c.name as channel_name 
                       FROM messages m 
    ...
    if ($res) {
        while ($row = $res->fetchArray(SQLITE3_ASSOC)) {
            echo "[{$row['created_at']}] Channel: {$row['channel_name']}, Direction: {$row['direction']}, Body: {$row['body']}\n";
        }
    } else {
