...
const STAGES = [
  ...
  { status: 'Application Withdraw', hex: '#dc2626', col: 'bg-red-700', border: 'border-red-700', light: 'bg-red-50', ring: 'ring-red-200/50' },
];
  ...
  const [filters, setFilters] = useState(() => {
    ...
    if (saved) {
      try {
        ...
        if (searchParams.get('status')) parsed.status = searchParams.get('status');
        return parsed;
      } catch (e) {}
    }
    return {
...
function FilterSelect({ value, onChange, options, placeholder, formatOption }) {
  return (
