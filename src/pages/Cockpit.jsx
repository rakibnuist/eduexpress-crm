/* Executive Cockpit — the remote-owner command center.
...
import {
  ...
  CircleCheck, CircleDot, UserPlus, TagIcon, Receipt, MapPin, Radio,
  Zap, Shield, Bell, FileWarning, Siren, ArrowUpRight, ArrowDownRight,
  BarChart3, Flame, GitPullRequest, CalendarDays
} from 'lucide-react';
import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, Bar, BarChart, Area, AreaChart, CartesianGrid } from 'recharts';
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold tracking-tight">Executive Cockpit</h1>
                <p className="text-sm text-slate-400 mt-0.5">
                  {new Date().toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long' })}
                  {' · '}<span className="text-emerald-400 font-semibold">{new Date().toLocaleTimeString('en-GB', { timeZone: 'Asia/Dhaka', hour: '2-digit', minute: '2-digit' })} Dhaka</span>
                  {' · '}<span className="text-xs bg-red-500/20 text-red-400 px-2 py-0.5 rounded-full font-bold">LIVE</span>
                </p>
              </div>
            </div>
            <button onClick={load} disabled={refreshing}
              className="flex items-center gap-2 text-xs font-medium bg-white/10 hover:bg-white/20 text-white px-4 py-2.5 rounded-xl border border-white/10 transition-all">
              <RefreshCw size={13} className={refreshing ? 'animate-spin' : ''} />
              {lastSync ? `Synced ${timeAgo(lastSync.toISOString())}` : 'Refresh'}
            </button>
          </div>

          {/* Pulse cards in header */}
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 mt-6">
            <CommandCard icon={<Users size={16} />} color="emerald" label="In office today"
              value={`${today.attendance.checkedIn.length}/${today.attendance.totalActive}`}
              sub={today.attendance.missing.length === 0 ? 'Everyone in ✓' : `${today.attendance.missing.length} missing`} />
            <CommandCard icon={<UserPlus size={16} />} color="blue" label="New leads today"
              value={today.stats.newLeads}
              sub={`Yesterday: ${yesterday.stats.newLeads}`} />
            <CommandCard icon={<CheckCircle2 size={16} />} color="violet" label="Conversions today"
              value={today.stats.conversions}
              sub={`Yesterday: ${yesterday.stats.conversions}`} />
            <CommandCard icon={<DollarSign size={16} />} color="amber" label="Money in today"
              value={fmt(today.stats.paymentsAmount)}
              sub={`${today.stats.paymentsCount} payment${today.stats.paymentsCount === 1 ? '' : 's'}`} />
            <CommandCard icon={<AlertTriangle size={16} />} color={totalAlerts > 0 ? 'rose' : 'slate'}
              label="Alerts"
              value={totalAlerts}
              sub={totalAlerts === 0 ? 'All clear' : `${totalAlerts} items need attention`} />
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/60 flex items-center justify-between">
              <div>
                <h3 className="font-bold text-slate-700 flex items-center gap-2"><TrendingUp size={15}/> 7-Day Lead Trend</h3>
                <p className="text-xs text-slate-400 mt-0.5">New leads per day with revenue overlay</p>
              </div>
              <span className="text-xs font-semibold bg-blue-50 text-blue-600 px-2.5 py-1 rounded-full">Last 7 days</span>
            </div>
            <div className="p-6">
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={trend} margin={{ top: 0, right: 8, left: -10, bottom: 0 }}>
                  ...
                  <Bar dataKey="newLeads" fill="#3b82f6" radius={[6, 6, 0, 0]} name="New leads" maxBarSize={40} />
                </BarChart>
              </ResponsiveContainer>
              <div className="mt-5 pt-4 border-t border-slate-100">
                <p className="text-xs font-bold text-slate-500 mb-3 flex items-center gap-1.5">
                  <DollarSign size={12} className="text-emerald-500"/> Daily revenue collection
                </p>
                <ResponsiveContainer width="100%" height={80}>
                  <AreaChart data={trend} margin={{ top: 4, right: 8, left: -10, bottom: 0 }}>
                    <defs>
                      <linearGradient id="cockpitRev" x1="0" y1="0" x2="0" y2="1">
                    ...
                    <Area type="monotone" dataKey="revenue" stroke="#10b981" strokeWidth={2.5} fill="url(#cockpitRev)" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/60">
              <h3 className="font-bold text-slate-700 flex items-center gap-2"><CalendarClock size={15}/> Yesterday's Recap</h3>
              <p className="text-xs text-slate-400 mt-0.5">{yesterday.date}</p>
            </div>
            <div className="p-6 space-y-3">
              <RecapStat label="Attendance" value={`${yesterday.attendance.checkedIn.length}/${yesterday.attendance.totalActive}`} />
              <RecapStat label="New leads" value={yesterday.stats.newLeads} />
              <RecapStat label="Conversions" value={yesterday.stats.conversions} />
              <RecapStat label="Money in" value={fmt(yesterday.stats.paymentsAmount)} highlight="pos" />
              <RecapStat label="Money out" value={fmt(yesterday.stats.expensesAmount)} highlight="neg" />
              <div className="pt-3 mt-2 border-t border-slate-100">
                <RecapStat label="Net cash" value={fmt(yesterday.stats.netCash)} highlight={yesterday.stats.netCash >= 0 ? 'pos' : 'neg'} bold />
              </div>
            </div>
          </div>
        </div>
...
/* ── pieces ─────────────────────────────────────────────────────────────── */
function CommandCard({ icon, label, value, sub, color }) {
  const gradients = {
  ...
  return (
    <div className={`rounded-xl p-3.5 border ${gradients[color] || gradients.slate} backdrop-blur-sm`}>
      <div className="flex items-center gap-2 mb-2">
        <div className={icons[color]}>{icon}</div>
        <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">{label}</p>
      </div>
      <p className="text-2xl font-extrabold text-white leading-tight">{value}</p>
      <p className="text-[11px] text-slate-400 truncate mt-1">{sub}</p>
    </div>
  );
}

function RecapStat({ label, value, highlight, bold }) {
  const colorMap = { pos: 'text-emerald-600', neg: 'text-rose-600' };
  return (
    <div className="flex justify-between items-center py-1">
      <span className="text-xs text-slate-500 font-medium">{label}</span>
      <span className={`text-sm font-${bold ? 'extrabold' : 'bold'} ${colorMap[highlight] || 'text-slate-800'}`}>{value}</span>
    </div>
  );
}
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs font-semibold bg-slate-100 text-slate-500 px-2.5 py-1 rounded-full">{data.date}</span>
      </div>
      <div className="space-y-4">
        <div>
          <p className="text-xs font-bold text-slate-500 mb-2 uppercase tracking-wide">Checked in ({checkedIn.length})</p>
          {checkedIn.length === 0 ? (
            <p className="text-sm text-slate-400 italic">Nobody yet</p>
          ) : (
            <div className="space-y-1.5">
              {checkedIn.map(a => (
                <div key={a.emp_id} className={`flex items-center justify-between p-2.5 rounded-lg text-sm
                  ${a.status === 'Late' ? 'bg-amber-50 border border-amber-100' : 'bg-emerald-50 border border-emerald-100'}`}>
                  ...
                  <div className="text-xs text-slate-500 flex-shrink-0 font-mono">
                    ...
                    {a.hours_worked ? <span className="ml-1 font-bold">({Number(a.hours_worked).toFixed(1)}h)</span> : ''}
                  </div>
                </div>
              ))}
            </div>
        ...
        {missing.length > 0 && (
          <div>
            <p className="text-xs font-bold text-slate-500 mb-2 uppercase tracking-wide">Not checked in ({missing.length})</p>
            <div className="space-y-1.5">
              {missing.map(m => (
                <div key={m.emp_id} className="flex items-center gap-2 p-2.5 rounded-lg bg-slate-50 border border-slate-100 text-sm">
  ...
  return (
    <div className="space-y-4">
      <AlertLane title="Visa deadlines (next 30 days)" icon={<CalendarClock size={16} />} color="rose"
        ...
        rows={visaDeadlines}
        renderMeta={l => <span className="text-xs font-bold text-rose-700 bg-rose-50 px-2 py-0.5 rounded-full">{l.visa_deadline}</span>}
      />
      ...
      />
      <AlertLane title="Unassigned leads" icon={<UserX size={16} />} color="rose"
      ...
      <AlertLane title="Follow-up overdue" icon={<CalendarClock size={16} />} color="rose"
        ...
        renderMeta={l => <span className="text-xs font-bold text-rose-600 bg-rose-50 px-2 py-0.5 rounded-full">due {l.next_followup}</span>}
      />
      <AlertLane title={`Outstanding balance — ${fmt(totalOutstanding)} total`} icon={<Wallet size={16} />} color="violet"
        ...
        renderMeta={l => <span className="text-xs font-bold text-violet-700 bg-violet-50 px-2 py-0.5 rounded-full">{fmt(l.balance)}</span>}
      />
    </div>
  );
}
...
function AlertLane({ title, icon, color, subtitle, rows, renderMeta }) {
  ...
  const palette = {
    amber:  { icon: 'bg-amber-50 text-amber-600 border-amber-100', badge: 'bg-amber-100 text-amber-700', border: 'border-amber-200', bg: 'bg-amber-50/30' },
    rose:   { icon: 'bg-rose-50 text-rose-600 border-rose-100',    badge: 'bg-rose-100 text-rose-700',   border: 'border-rose-200', bg: 'bg-rose-50/30' },
    violet: { icon: 'bg-violet-50 text-violet-600 border-violet-100', badge: 'bg-violet-100 text-violet-700', border: 'border-violet-200', bg: 'bg-violet-50/30' },
  };
  ...
  return (
    <div className={`bg-white rounded-2xl border ${hasItems ? p.border : 'border-slate-200'} overflow-hidden shadow-sm`}>
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center gap-3 px-6 py-4 hover:bg-slate-50/60 transition-colors text-left">
        <div className={`p-2.5 rounded-xl border ${p.icon} flex-shrink-0`}>{icon}</div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <p className="font-bold text-slate-800 text-sm">{title}</p>
            <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full ${hasItems ? p.badge : 'bg-slate-100 text-slate-500'}`}>
              {rows.length}
            </span>
          </div>
            ...
            <div className="divide-y divide-slate-50">
              {rows.map(l => (
                <Link key={l.id} to={`/leads?q=${encodeURIComponent(l.lead_id || l.client_name)}`}
                  className="flex items-center gap-3 px-6 py-3.5 hover:bg-blue-50/30 transition-colors group">
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-slate-800 truncate group-hover:text-blue-700 transition-colors">{l.client_name}</p>
...
function FeedTab({ feed }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
      <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/60 flex items-center justify-between">
        <div>
          <h3 className="font-bold text-slate-700 text-sm">Live Activity Feed</h3>
          <p className="text-xs text-slate-400">Auto-refreshes every 30 seconds</p>
        </div>
        <span className="flex items-center gap-1.5 text-[11px] text-emerald-600 font-bold bg-emerald-50 px-3 py-1.5 rounded-full border border-emerald-100">
...
function FeedItem({ a }) {
  ...
  return (
    <div className="flex items-start gap-3 px-6 py-3.5 hover:bg-slate-50/60 transition-colors group">
      <div className={`p-2 rounded-xl flex-shrink-0 ${meta.bg}`}>{meta.icon}</div>
      <div className="flex-1 min-w-0">
        <p className="text-sm text-slate-700 leading-snug">
          <strong className="text-slate-800 font-bold">{a.actor_name || 'System'}</strong>{' '}
          {meta.verb}
          {a.lead_name && <Link to={`/leads?q=${encodeURIComponent(a.lead_name)}`} className="text-blue-600 hover:underline font-bold ml-1">{a.lead_name}</Link>}
      ...
      <span className="text-[11px] text-slate-400 flex-shrink-0 tabular-nums font-mono">{timeAgo(a.created_at)}</span>
    </div>
  );
}
